...
//     let exists = database.some(database => database.email === email);
//     return !exists;
// }
...
//             # Logout:
let money=0
let banque=prompt("choise:logout,withraw money,deposit money,take a loan , invest,history")
if (banque=== "logout") {
    alert("rak khrjty")
        } 
        else{
            if ( banque=== "withraw money") {
            let chhal=prompt("chhal bghi tkharj")
            if(chhal>money){
                alert("rah ma3ndkch flos")
            }else{
                money-=chhal
                alert("chhal b9a andk"+ money)
            }
        
        }
        if (banque=="deposit money") {
            let dakhil=prompt("chhal baghi dakhl dayl flos")
            if(dakhil<1000){

                money+=dakhil
            }else{
                alert("ma3ndkch ha9 dakhl ktar mn 1000 dh")
            }
        }
        }
